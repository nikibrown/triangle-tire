...
foreach( $all_taxonomies as $slug => $label )
{
	...
	if( !empty( $cats ) )
	{
		foreach( $cats as $c )
		{
			...
			$link = '<a href="'. get_bloginfo( 'url' ) .'/'. $c->taxonomy .'/'. $c->slug.'">'. $c->name .'</a>';
			$catlist[] = $link;

			if( $c->taxonomy == 'radialbias' )
			{
				$has_radial_bias = $c->name;
			}
		}
	}
...
<div class="js-printblock">
	...
	<div class="row">
		<div class="medium-7 large-9 columns">
			<h1>
				<a href="<?php bloginfo( 'url' ); ?>/<?php echo $this_link; ?>">
					<?php echo $this_label; ?>
				</a>
			</h1>
		</div>
	...
	<div class="tirelist productdetail row" data-equalizer>
		...
		<div class="medium-8 large-8 columns" data-equalizer-watch>
			<div class="tireheaderwrap">
				<h2><?php the_title(); ?> <span class="industry-code"><?php the_field('custom_industry_code'); ?></span></h2>
				...
				<?php } ?>
				<?php
				if( $has_radial_bias !== false )
				{
					echo '<div class="additionaltireinfo">';
						echo  ( isset( $nl ) && $nl ? '<br>' : '' ) . $has_radial_bias;
					echo '</div>';
				}
				?>
			</div>
